...
let bedrockStartCalled = false;
bedrock.events.on('bedrock.start', () => {
  bedrockStartCalled = true;
});
...
function _splitKeyId({id}) {
  ...
  // convert local IDs that start with `z` to `Buffer` for storage savings
  // (`z<base58-encoded ID>`)
  // where the ID is a multicodec (multihash) value,
  // either encoded 16 byte random value:
  // 0x00 = identity tag, 0x10 = length (16 bytes) header
  // or (another value that can't be confused with the first option):
  // <some other multicodec header><value>
  let compressedLocalId = localId;
  if(localId.startsWith('z')) {
    const mh = base58.decode(localId.slice(1));
    if(mh[0] === 0 && mh[1] === 16 && mh.length === 18) {
      // max compression for 16-byte raw (identity hash) values
      compressedLocalId = Buffer.from(mh.slice(2));
    } else if(mh.length > 16) {
      // otherwise, preserve multihash header
      compressedLocalId = Buffer.from(mh);
    }
  }
  return {
    keystoreId: id.slice(0, idx),
    localId: compressedLocalId
  };
}
